...
class EnvanterSayfasi(ctk.CTkFrame):
    ...
    def stok_guncelle_penceresi(self, malzeme_id):
        ...
        def satir(isim, m_val, b_val):
            ...
            # Eski birim listede yoksa bile (dil değişimi sonrası) atamasını yap
            c.set(b_val) 
            c.pack(side="left")
            return e, c
        ...
        def kaydet():
            try:
                ...
                # DINAMIK GUNCELLEME LOGU
                dil = self.app.aktif_dil
                islem_metni = f"{metinler[dil]['log_guncellendi']}"
                
                log_id = self.db.gecmis_ekle(malzeme_id, eski_m.isim, islem_metni, self.app.kullanici_adi)
    ...
    def malzeme_sil_islemi(self, malzeme_id):
        ...
        log_id = self.db.gecmis_ekle(malzeme_id, m.isim, islem_metni, self.app.kullanici_adi)
