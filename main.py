...
class EnvanterUygulamasi(ctk.CTk):
    ...
    def kullanici_listesini_ciz(self):
        ...
        if not kullanicilar: ctk.CTkLabel(self.kullanici_frame, text=metinler[self.aktif_dil]["kayitli_yok"], text_color=renkler["yazi_ikincil"]).pack(pady=20)
            ...
            for k in kullanicilar:
                ...
                ctk.CTkButton(row, text="X", width=30, fg_color=renkler["tehlike"], command=lambda isim=k: self.db.kullanici_sil(isim) or self.kullanici_listesini_ciz()).pack(side="right", padx=5, pady=5)
    ...
    def islemi_ileri_al(self):
        ...
        if islem.tip == "ekleme":
            ...
            islem_metni = metinler[dil]['log_guncellendi']
            islem.log_id = self.db.gecmis_ekle(islem.malzeme_id, islem.malzeme_isim, islem_metni, self.kullanici_adi)
